...
scaled_deplyoments = ["mlrun-api-chief","mlrun-db","mlrun-jupyter","mlrun-ui","mpi-operator","nuclio-controller","nuclio-dashboard"]
valid_registry_args = [
    "kind",
    "server",
    "username",
...
def stop(env_file, deployment, cleanup, force, verbose):
    ...
    if not deployment:
        logging.error("cannot determine current deployment type, please specify the -d option")
        return
...
@click.option(
    ...
    default=[],
    multiple=True,
    help=f"optional services to disable (end with '-' to disable), supported services: {','.join(optional_services)}",
)
@click.option(
    "--set",
    "-s",
...
class BaseConfig:
    ...
    def do_popen(self, cmd, stdin=None,env=None, interactive=True):
        if self.simulate:
        ...
        if self.verbose:
            print(cmd)
        try:
            child = subprocess.Popen(cmd, env=env,stdin=stdin, stdout=output, stderr=output)
...
class K8sConfig(BaseConfig):
    ...
    def start(
        ...
        **kwargs,
        ...
        if tag:
            logging.info(f"MLRun tag detected: {tag} ")
        tag = tag or get_latest_mlrun_tag()
        ...
        if self.check_k8s_resouce_exciting("namespace",namespace):
            ...
            text = click.prompt(
                "To overwrite the existing namespace press y or Y", type=str, default="n"
            )
            text = text.lower()
            if "y" in text:
    ...
    def parse_services(include):
        ...
        if include:
            for service in include:
                ...
                extra_sets.append(service_map[service[0]] + f".enabled=false")
        return extra_sets
    ...
    def configure_registry(self, namespace, registry_args):
        ...
        if self.check_k8s_resouce_exciting(namespace=namespace,resource="secret",name=pull_secret):
            ...
            text = click.prompt(
                "To overwrite the existing Registry Secret press y or Y", type=str, default="n"
            )
            text = text.lower()
            if "y" in text:
                returncode, _, _ = self.do_popen(["kubectl","-n",namespace,"delete","secret",pull_secret])
                if returncode != 0:
                    raise SystemExit(returncode)
            else:
                create_secret=False
